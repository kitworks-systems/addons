...
class TestCrmLead(TransactionCase):
    ...
    def test_post_write_stage_id_sms_phone_fallback_logic(self):
        ...
        # Тест 2: Fallback до partner.mobile
        partner_mobile_only = self.partner_model.create({
            'name': 'Partner Mobile Only',
            'mobile': '+8888888888',
            'phone': False,
        })

        lead_mobile_fallback = self.lead_model.create({
            'name': 'Lead Mobile Fallback',
            'partner_id': partner_mobile_only.id,
            'phone': False,
            'kw_send_message': False,
        })

        lead_mobile_fallback.write({'stage_id': stage_with_sms.id})
        self.assertTrue(lead_mobile_fallback.kw_send_message)

        # Тест 3: Fallback до partner.phone
    ...
    @patch('odoo.addons.sms.models.sms_sms.SmsSms.send')
